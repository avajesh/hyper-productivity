...
This reference compares the **sync providers** available in Hyper Productivity: how each stores and syncs your data, which platforms they support, how authentication works, and whether optional encryption is available. Use it to choose a sync provider or to understand how your data moves between devices.
...
Sync providers **synchronize your full Hyper Productivity data** (tasks, projects, tags, time tracking, settings, archives, and so on) between this app instance and a remote location or another device. They do **not** import issues from Jira, GitHub, or other issue trackers; those are handled by issue providers. All sync in the app is **local-first** and **operation-based**: your device holds the primary copy, and changes are sent as operations (or a sync file containing state and operations) to the remote side. You configure one sync provider at a time.
...
- **What it is:** A dedicated sync server designed for Hyper Productivity. Can be self-hosted or used with a hosted instance. **SuperSync is very new and is still in beta;** consider Nextcloud, WebDAV, Dropbox, or local file for production use until it is stable.
