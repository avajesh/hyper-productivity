...
the
[authentication authority](https://github.com/avajesh/hyper-productivity/blob/master/packages/super-sync-server/docs/authentication.md).
The
[executable routes](https://github.com/avajesh/hyper-productivity/blob/master/packages/super-sync-server/src/api.ts)
...
Use the
[Sync Server architecture](https://github.com/avajesh/hyper-productivity/blob/master/packages/super-sync-server/docs/architecture.md)
for endpoint purposes and executable owners. Exact routes, limits, error codes,
and wire schemas live in
[`sync.routes.ts`](https://github.com/avajesh/hyper-productivity/blob/master/packages/super-sync-server/src/sync/sync.routes.ts)
and the
[shared HTTP contract](https://github.com/avajesh/hyper-productivity/blob/master/packages/shared-schema/src/supersync-http-contract.ts).
...
[`PluginManifest`](https://github.com/avajesh/hyper-productivity/blob/master/packages/plugin-api/src/types.ts)
...
The Local REST API allows external scripts and tools to interact with a running Hyper Productivity desktop app. It runs on `http://127.0.0.1:3876` and is disabled by default. Enable it in **Settings → Misc → Enable local REST API**.
...
- **The API does not authenticate itself to clients.** The port is fixed and unprotected,
  so a process that binds `127.0.0.1:3876` while Hyper Productivity is _not_ listening —
...
| ------------------- | ------------------------------- | ----------------------------------------------------------- |
| iOS (Capacitor)     | `com.hyper-productivity.app://` | `com.hyper-productivity.app://create-task?title=Buy%20milk` |
| Android (Capacitor) | `com.hyper-productivity.app://` | `com.hyper-productivity.app://create-task?title=Buy%20milk` |
...
```text
com.hyper-productivity.app://create-task?title=Buy%20milk&notes=2%25%20fat&projectId=proj-1
com.hyper-productivity.app://complete-task?title=Buy%20milk
...
- **Siri (iOS):** create a Shortcut in the Apple Shortcuts app — "Ask for Input" (text) followed by "Open URLs" with `com.hyper-productivity.app://create-task?title=` and the input appended — then invoke it by name: "Hey Siri, \<shortcut name\>". Siri dictates the title, the URL opens the app, and the task is added. Query parameter values must be URL-encoded; Shortcuts' "URL Encode" action handles titles with spaces or special characters.
