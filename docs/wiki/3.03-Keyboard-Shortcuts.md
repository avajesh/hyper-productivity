...
Hyper Productivity supports an extensive keyboard shortcut system. Shortcuts are grouped by function, and most are user-configurable via [[3.02-Settings-and-Preferences]].
...
On Wayland the compositor — not the application — owns global hotkeys, so the built-in global shortcuts above may not fire. Hyper Productivity registers the `superproductivity://` URL scheme, and the already-running instance handles these URLs, so you can bind compositor keys to them with the standard `xdg-open` command (part of `xdg-utils`, already present on virtually every Linux desktop — no extra tool or runtime to install).
...
If `xdg-open` does not reach the app, confirm the desktop entry registers the scheme with `xdg-mime query default x-scheme-handler/superproductivity` (it should print Hyper Productivity's `.desktop` file). The same mechanism works on X11; it is only required on Wayland.
