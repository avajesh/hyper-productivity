...
Sync backs up and synchronizes your full Hyper Productivity data (tasks, projects, time tracking, settings, archives, and so on) between this app and a remote location or another device. You configure **one** sync provider per device; each device can use a different provider (the **Sync provider** setting is not synced between devices). For a detailed comparison of providers, see [[3.08-Sync-Integration-Comparison]]. For how to configure the one you choose, see [[2.09-Configure-Sync-Backend]].

It is not a requirement to use any sync backend if you only use Hyper Productivity on a single device. Ensure that you have backups configured and that you know how to [[2.02-Restore-Data-From-Backup]] in case of data corruption from other errors.
