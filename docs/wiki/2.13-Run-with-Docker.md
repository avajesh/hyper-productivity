...
How to run the Hyper Productivity web app in Docker, and how to use the WebDAV integration.

## Run the Web App only

```bash
docker run -d -p 80:80 avajesh/hyper-productivity:latest
```
...
1. Copy the repo files to a folder (e.g. `sp/`):
   ...
   mkdir -p sp
   cp hyper-productivity/docker-compose.yaml sp/
   cp hyper-productivity/webdav.yaml sp/
   cd sp
   ```
